// “ALL OF YOU BEHIND ME, WE'RE GOIN' IN!”
	...
	GatherParticipants = function( self )
		...
		if tAllies then
			...
			for ent in pairs( tAllies ) do
				if !IsValid( ent ) || !ent.__ACTOR__ || ent:GetPos():DistToSqr( vPos ) > 9437184/*3072*/ || ent.Schedule && ent.Schedule.bFromCombatFormation then continue end
				local pPath = ent.pLastEnemyPath
				if pPath then if !ent:ShouldStackUp( pPath ) then continue end end
